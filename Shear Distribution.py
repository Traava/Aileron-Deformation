...
def HorizontalShear(Sz):
    ...
    x1 = np.linspace(0,theta_separation,50)
    qb11 = -(1/Izz_Aileron)*((integrate(function(x1),x1))*r*t_sk) + qb01
    x2 = np.linspace(theta_separation,pi/2,50)
    qb12 = -(1/Izz_Aileron)*(((integrate(function(x2),x2))*r*t_sk) + boom_area*(y_b1+y_b2)) + qb11[-1]
    ...
    qbz2 = -(1/Izz_Aileron)*((integrate(y,x))*t_sp) + qb02
    ...
    x = np.linspace(0,r,50)
    y = np.ones(len(x))*(r-centroid)
    qbz5 = -(1/Izz_Aileron)*((integrate(function(x),x))*t_sp) + qb05
    ...
    x = np.linspace(-pi/2,-theta_separation,50)
    qb61 = -(1/Izz_Aileron)*((integrate(function(x),x))*r*t_sk)  + qb06
    x = np.linspace(-theta_separation,0,50)
    qb62 = -(1/Izz_Aileron)*((integrate(function(x),x))*r*t_sk + boom_area*(y_b11))  + qb61[-1]
